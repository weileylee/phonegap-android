...
Contacts.prototype.getAllContacts = function(success, error){
	this.records = [];
...
Contacts.prototype.newContact = function(obj, success, options){
	ContactHook.addContact(obj.name, obj.phone, obj.email);
	this.win = success;
}

Contacts.prototype.droidDidCreateContact = function(name, phone, email){
	this.win({result:'ok','name':name, 'phone':phone, 'email':email});
}

Contacts.prototype.droidFailCreateContact = function(){
	this.win({result:'error'});
}
...
DebugConsole.prototype.log = function(message){
	if(ConsoleHook){
		ConsoleHook.log(this.processMessage(message));
	}else{
		if(console){
...
DebugConsole.prototype.debug = function(message){
	if(ConsoleHook){
    ConsoleHook.debug(this.processMessage(message));
  }else{
    if(console){
      console.log(messsage);
    }
...
DebugConsole.prototype.error = function(message){
  if(ConsoleHook){
    ConsoleHook.error(this.processMessage(message));
  }else{
    if(console){
      console.log(messsage);
    }
